...
type Retirement = { target_cny: number; current_cny: number; progress: number; projected_years: number | null; annual_rate: number; items: RetirementItem[] };
...
export default function Dashboard() {
  ...
  useEffect(() => {
    fetch("/api/dashboard")
      ...
      .then((dashboard) => {
        ...
        void fetch("/api/retirement").then((response) => response.ok ? response.json() as Promise<Retirement> : null).then((goal) => { if (goal) setRetirement(goal); });
      })
      .catch(() => setUser(null))
      .finally(() => {
  ...
  async function saveRetirement(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/retirement", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: form.get("name"), category: form.get("category"), currency: form.get("currency"), amount: Number(form.get("amount")) }) });
      const data = await response.json() as Retirement & { error?: string };
        ...
        <section className="forecast-card" id="forecast">
          <div className="forecast-copy">
            ...
            <form className="retirement-form" onSubmit={saveRetirement}>
              ...
              {(retirement?.items ?? []).length > 0 && <ul className="retirement-items">{retirement!.items.map(item=><li key={item.id}><span>{item.name} · {item.category}</span><b>{money(item.currency === "CNY" ? item.amount : 0)} {item.currency}</b><button type="button" onClick={()=>void removeRetirementItem(item.id)}>删除</button></li>)}</ul>}
            </form>
