  ...
  async function POST(request: Request) {
    try {
      ...
      const body = (await request.json()) as Partial<{
        name: string; category: string; code: string; amount: number; currency: string; annualRate: number; note: string;
      }>;
      ...
      ).bind(user.id, name, category, code, amount, currency, annualRate, note).first<AssetRow>();
  ...
  async function PATCH(request: Request) {
    try {
      ...
      if (body.amount !== undefined || body.currency !== undefined) {
        ...
        const result = await db.prepare("UPDATE assets SET amount = ?, currency = ? WHERE id = ? AND user_id = ?")
          .bind(amount, currency, id, user.id).run();
        if (!result.meta.changes) return Response.json({ error: "资产不存在" }, { status: 404 });
        return Response.json({ ok: true, amount, currency, snapshot: await snapshot(db, user.id) });
      }
